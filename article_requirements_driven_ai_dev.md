...
### AIのユニットテストは「自己採点」になる

コードレビューと同じ問題が自動テストにも起きます。AIにコードとユニットテストを同時に生成させると、**実装を見てからテストを書く**ため、テストは必ず通ります。これは検証ではなく自己採点です。

有効なのは**要件定義書に対応した結合テスト**です。「貸出操作をすると備品のステータスが貸出中になること」のように、要件定義書という外部仕様を根拠にすれば、AIが実装を見て帳尻合わせすることができません。純粋な数値計算・データ変換のユニットテストは例外的に有効です。

### 仕様の変更点だけテストを作ればいい

テストの本来の目的は**リグレッション検出**です。改良のたびに既存機能が壊れていないかを確認するために、テストを積み上げていきます。

仕様駆動開発では、改良時に「要件定義書の変更点」を起点にコードを生成するため、**変更の影響範囲が仕様レベルで明確**です。コードのどこが変わったか追わなくても、仕様の差分を見れば影響範囲がわかります。そのため、変更箇所の周辺の結合テストだけ生成すれば十分で、全体にテストを積み上げる必要はありません。

```text
従来：コードにパッチ → どこが変わったか不明 → 全体にテストを積み上げる
仕様駆動：要件定義書の変更点 → 変更箇所のコード生成 → その周辺の結合テストだけ生成
```

また、**テスト駆動開発（TDD）もこの手法には合いません**。AIにテストを先に書かせてからコードを生成させると、今度は「テストを通すためのコード」を書くことになります。テストもコードもAIが生成する以上、結局は自己採点です。外部仕様である要件定義書を根拠にした結合テストを、コード生成の後に作るのが正しい順序です。

### 今後の展望：仕様差分テスト

現在公開しているプロンプトはコード生成までですが、将来的には**要件定義書の変更前後の差分から結合テストを自動生成する**プロンプトを追加したいと考えています。

これが成立する理由は、要件定義書の各要件が「テストすべき振る舞いの一覧」そのものだからです。差分を取れば「何をテストすべきか」が自動で決まり、「差分の全要件の結合テストが通ればDone」という明確な完了定義にもなります。これは**仕様ベーステスト（Specification-based Testing）** と呼ばれる確立された手法と同じ本質で、仕様駆動開発のサイクルを完結させる自然な拡張です。
